...
class OpenRouterLLM(LLMBase):
    def __init__(self, config: Optional[BaseLlmConfig] = None):
        ...
        # 获取 OpenRouter 的配置
        openrouter_config = next(p for p in LlmProvidersConfig["providers"] if p["name"] == "openrouter")

        if not self.config.model:
            self.config.model = "gpt-4o-mini"

        # 使用配置中的 base_url
        self.client = OpenAI(base_url=openrouter_config["base_url"])
        # 获取模型的 max_tokens
        self.max_tokens = next(
            m["max_tokens"] for m in openrouter_config["models"] if m["name"] == self.config.model
        )
